...
# Function to generate PDF without using an HTML template
@csrf_exempt
...
def generate_pdf_tamil_new(request):
    tamil_text = "வணக்கம், இது தமிழில் ஒரு PDF உள்ளடக்கம்!"
    english_text = "Hello, this is a PDF in Tamil!"

    # Ensure the font path is absolute
    font_path = os.path.abspath(os.path.join(settings.BASE_DIR, 'fonts', 'NotoSansTamil-VariableFont_wdth,wght.ttf'))
    
    translated_text = GoogleTranslator(source='en', target='ta').translate("Hello, this is a PDF in Tamil!")

    # Inline HTML content with proper font embedding
    html_content = f"""
    <!DOCTYPE html>
    <html lang="ta">
    <head>
